...
## M1b — local browser desktop (complete prototype)

The local browser prototype is proven on the six-host alpha fleet at 1080p
H.264 with real keyboard/mouse control. It remains unauthenticated and
single-device; it is not the fleet product or an internet endpoint.
...
## M2 — authenticated fleet web console (next milestone)

Build the original React console and .NET broker around the proven daemon. Use
Keycloak OAuth/OIDC authorization-code + PKCE, server-side sessions and a
device registry. The first useful slice is: sign in, list enrolled devices
with online/capability status, create one session, view/control it, and destroy
it. The broker owns identity, authorization, grants and fleet routing; daemons
retain capture, input leases and OS policy. Keep it internal or behind UniFi
VPN until security acceptance. Native source resolution and active-session
resize remain mandatory; browser zoom is not a substitute.

## Later stages

M3 exposes the same sessions through MCP with human takeover. M4 provisions
computer-use desktops through DonkeyWork Sandbox, including Vault-backed
credential paste. WP10 physical/VKMS hotplug remains a separate follow-up.
...
Actual remote desktop resolution must change during an active session: via CLI
in M1a and via web UI in M2. This is not browser scaling or encoder
downsampling. Both demonstrate 3840×2160 -> 1920×1080 -> 3840×2160 without
reconnecting the desktop or disrupting the terminal. Unsupported resize on the
selected pilot blocks completion.
