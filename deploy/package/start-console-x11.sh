...
/opt/donkeywork-desktop/current/bin/dwconsole-daemon \
    --socket /run/donkeywork-desktop-capture/stream.sock --device "$drm_device" \
    --fps 30 --encoder libx264 --ffmpeg "$ffmpeg_path" --x11-display "$display" \
