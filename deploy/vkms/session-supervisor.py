"""Pilot GNOME/Wayland seat supervisor. Never restarts GDM or user applications.
...
UNITS = ('dwconsole-input-guard', 'dwconsole-vkms-output')
...
class Supervisor:
    ...
    def start_unit(self, name, command, properties=(), environment=()):
        ...
        self.command(['systemd-run', '--quiet', '--unit=' + name]
    ...
    def start_web(self, control):
        args = self.args
        if self.active('dwconsole-vkms-web'):
            return
        command = [str(args.bin_dir / 'web-launch-input'), '--socket', args.capture_socket,
                   '--capture-broker-socket', args.broker_socket,
                   '--executable', str(args.bin_dir / 'console-web-input'), '--uid', '65534', '--gid', '65534']
        if control:
            command += ['--input-socket', args.input_socket]
        command += ['--', '--stream-fd', '0', '--listen', args.host_ip + ':8090',
                    '--capture-broker-fd', '4',
                    '--ice-listen', args.host_ip + ':8091', '--origin', 'http://' + args.host_ip + ':8090',
                    '--assets', str(args.assets)]
        if control:
            command += ['--input-fd', '3']
        self.start_unit('dwconsole-vkms-web', command,
                       ['Restart=always', 'RestartSec=2', 'NoNewPrivileges=yes',
    ...
    def prepare(self, target):
        ...
        args = self.args
        self.stop_units()
        # Establish the VKMS layout only at the greeter. Reapplying
        # ApplyMonitorsConfig inside a logged-in Mutter session can race its
        # llvmpipe renderer during GDM handoff and crash gnome-shell. The
        # user session inherits the greeter's already-active Virtual-1 mode.
        if session_class == 'greeter':
            self.start_unit('dwconsole-vkms-output', ['/usr/bin/python3',
                            str(args.bin_dir / 'keep-console-output.py')],
        ...
        self.start_unit('dwconsole-input-guard', ['/usr/bin/python3',
            str(args.bin_dir / 'input-target-guard.py'), '--session', session,
            '--session-uid', str(uid), '--session-bus', address, '--drm-state', str(args.drm_state),
            '--output', args.guard_state], ['RuntimeDirectory=dwconsole-guard', 'RuntimeDirectoryMode=0700'])
        self.wait_ready(target, lambda: valid_guard(args.guard_state, session, guard_started))
        if not self.active('dwconsole-input-probe'):
            self.start_unit('dwconsole-input-probe', [str(args.bin_dir / 'input-daemon'),
                '--socket', args.input_socket, '--width', '1920', '--height', '1080',
                '--guard-state', args.guard_state], ['RuntimeDirectory=dwconsole-input', 'RuntimeDirectoryMode=0700',
                                                   'NoNewPrivileges=yes'])
        def socket_ready():
            try:
        ...
        if not self.active('dwconsole-capture-broker'):
            self.start_unit('dwconsole-capture-broker', [str(args.bin_dir / 'socket-broker'),
                '--socket', args.broker_socket, '--capture-socket', args.capture_socket],
                ['RuntimeDirectory=dwconsole-broker', 'RuntimeDirectoryMode=0700', 'NoNewPrivileges=yes'])
...
def main():
    ...
    parser.add_argument('--bin-dir', type=Path, default=Path('/opt/donkeywork-desktop/bin'))
    parser.add_argument('--assets', type=Path, default=Path('/opt/donkeywork-desktop/web'))
    parser.add_argument('--drm-state', type=Path, default=Path('/sys/kernel/debug/dri/2/state'))
    parser.add_argument('--capture-socket', default='/run/dwconsole/stream.sock')
    parser.add_argument('--input-socket', default='/run/dwconsole-input/input.sock')
    parser.add_argument('--guard-state', default='/run/dwconsole-guard/state.json')
    parser.add_argument('--broker-socket', default='/run/dwconsole-broker/broker.sock')
    ...
    for path in (args.bin_dir, args.assets, args.drm_state, Path(args.capture_socket),
    ...
    if (Path(args.input_socket).parent != Path('/run/dwconsole-input')
            or Path(args.guard_state).parent != Path('/run/dwconsole-guard')
            or Path(args.broker_socket).parent != Path('/run/dwconsole-broker')):
        parser.error('input and guard paths must use their dedicated runtime directories')
    for name in ('keep-console-output.py', 'input-target-guard.py', 'input-daemon',
                 'web-launch-input', 'console-web-input', 'socket-broker'):
        info = (args.bin_dir / name).stat(follow_symlinks=False)
        if not stat.S_ISREG(info.st_mode) or info.st_uid != 0 or info.st_mode & 0o022:
            parser.error('pilot executable/script must be a root-owned non-writable regular file')
